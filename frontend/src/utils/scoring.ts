 ...
 *
 * ## ⚠️ 这里**没有**「与别人比」的那些东西（2026-09-23 整条移除）
 *
 * 原先这里有一个 `percentileOf(accuracy) = clamp(round(accuracy × 0.9), 5, 95)`
 * 的镜像实现，因为当时百分位只是「正确率的单调映射」。它后来改成按真实用户池算，
 * 再后来（同一天）**整条链路被撤掉** —— 原因不在算法，而在这个比较本身：
 * 「社团」在产品里没有任何数据实体，7 人池子上的 43% 是假精度，且拿「你这一局」
 * 比「别人的历史最佳」（还是别人另一份副本的最好成绩）并不成立。
 * 详见 `constants/copy.ts` 文件头第 4 条。
 *
 * 所以：
 *
 * - 本地 `QuizSummary` 不含任何比较字段，**也不该**加回来；
 * - 那一格现在讲「比过去的自己怎么样」，基准全部由服务端算
 *   （`backend/app/services/progress_service.py`），前端只查文案表
 *   （`progressTextOf`），一条比较逻辑都不写；
 * - 本地算的只有**本局**的判分与汇总（`summarizeQuiz`）—— 它是权威数字的
 *   即时反馈，服务端回来之后以服务端为准（见 `pages/settle`）。
 */
