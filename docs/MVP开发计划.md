...
> **后记（用户系统阶段）**：上表 **第 3 / 4 / 8 条已作废**。
> 三者是 MVP 阶段「不做用户体系」的前提假设，用户系统开工时被明确修订为
...
> **后记（用户系统阶段，2026-09-20）**：上表 **第 2 / 3 / 6 行已作废**，与 §1 表的
> 后记同源 —— 它们的前提同样是「MVP 阶段不做用户体系」：
> - 第 2 行（历史冒险日志）→ 已实现为 **04·5 历史卷轴列表 + 04·6 卷轴详情**（Phase E）。
> - 第 3 行（04 冒险者档案 9 屏）→ 已实现 **04·1/04·2/04·5/04·6/04·7/04·8 + 数据看板 + 知识树 + 勋章墙**（Phase D/E）。
> - 第 6 行（07 会员与设置 8 屏）→ 其中 **设置列表 / 头像与昵称 / 学习提醒 / 帮助与反馈 4 页**已实现（Phase E）；
>   会员与支付 4 屏仍不做。
>
> **仍然有效的两行**：分享海报（第 1 行，需 `getwxacodeunlimit`，本轮仍不做）、
> 05 卷轴工坊 / 06 公会社交（仍是 `ComingSoon` 占位页）。
> 修订依据见 `docs/用户系统需求分析文档.md §8`；本表**不在原地改写**，以保留决策演进的可追溯性。

> **后记二（私有知识库阶段，2026-09-24）**：第 4 行 **05 卷轴工坊** 也被切开 ——
> 与 §2.3 那个切法同源：
> - **已落地**：`05·2 上传文档` / `05·3 解析中` / `05·5 我的知识库` / `05·7 知识库详情` /
>   `05·8 从知识库出题` —— 另加一屏 `05·1 选择输入方式`（原型里是卷轴工坊 tab 的内容，
>   实现成独立页），共 **6 屏**，代码在 `frontend/src/pages/workshop/kb-*`。
> - **仍未落地**：`05·4 添加链接`（链接由既有的「用户给的链接必读」覆盖）、
>   `05·6 新建知识库`（收敛为「上传时自动建默认库 + 可改名」）、
>   库内**出题范围（按章节）**与**题型控件**（后端都没有控制面）。
> - 卷轴工坊 tab 因此**不再是空态**：它有了「我的知识库」与「选择输入方式」两个真入口。
> - 详细记录（页面 / 接口 / 配置 / 原型偏离 / 闸门）见本文 **§12.5**。
>
> **06 公会社交** 仍是 `ComingSoon`。
...
| 5.3 | 遗留清单：海报 / P1–P3 的接入点标注 |

> **5.3 现状（2026-09-20）**：历史日志与 04 档案、07 设置**均已落地**，不再属遗留（见 §3.2 后记）。
> 仅剩：`03 分享海报`（需 `getwxacodeunlimit`）、`05 卷轴工坊`、`06 公会社交`、
> `07 会员与支付 4 屏` —— 这四项在前端的接入点都是 `components/ComingSoon` 统一空态页，
> 新增功能时替换该组件即可。
>
> **(A) 离线缓存 —— 未做。这是「能力缺口」，不是「页面缺口」**（2026-09-21 记入）
>
> 原型 `03 第 7 屏` 的图注写明「断网时不阻塞浏览历史报告」，但本期**没有任何数据
> 离线缓存**：本地存储只用于登录态（`services/session.ts`）与设置页的「清理缓存」
> （`utils/cache.ts`，只统计/清理 `knowsaga.` 前缀键）。**断网时历史卷轴（04·5）
> 与报告一样打不开** —— 三个页面的入口都对，缺的是数据层这一块。
>
> - **当前的处理（诚实降级）**：`REPORT_COPY.offlineBody` 没有照抄原型那句
>   「已下载的冒险日志仍可查看」，改为如实说明。此属「与原型不一致」的第 3 处，
>   理由见 `frontend/src/constants/copy.ts` 文件头。断网出口本身是通的：
>   `splash` 启动探活失败 → `pages/exception/index` → 次按钮 `navigate` 到 04·5，
>   04·5 显示自己的失败态与「重新加载」（`components/ArchiveState`）。
> - **要做的事**：把最近 N 份卷轴 / 报告落本地（`Taro.setStorageSync` 或
>   `FileSystemManager`），断网时可读。**必须一并处理四件事**：①**换账号要清**
>   （存储键名带 `user_id`，否则 A 的日志会出现在 B 的设备上）；②过期策略；
>   ③体积上限（小程序 Storage 上限 10MB）；④与设置页「清理缓存」的口径统一
>   （现在那个只清 `knowsaga.` 前缀，缓存键要落在同一命名空间下才清得掉）。
> - **做完之后**：`offlineBody` 可回到原型原文，`exception` 页的次按钮
>   （「查看历史日志」）在断网时也才真正成立 —— 现在它仍只是个
>   「点进去看失败态」的出口。
...
### 7.2.2 `POST /quiz/generate` 的 `kb_id` 入参（2026-09-24）

请求体再多一个**可选**字段 `kb_id: int | None`：

```json
{ "user_input": "学习「机器学习导论」中的内容", "question_count": 3,
  "difficulty": "medium", "use_search": true, "kb_id": 7 }
```

四条契约：

1. **不传时请求与响应形状与接入知识库之前逐位一致** —— 这是「默认关 ⇒ 行为不变」
   那条红线的落点，`tests/test_quiz_kb.py` 里有一条专门钉它。
2. **归属校验在建任务之前**：越权与不存在返回**同一个 4005**，不会先给一个
   `task_id` 再让用户等几秒收到一个注定失败的任务（与 `TavilySearchProvider.__init__`
   里校验 key 同一条理由）。
3. **知识库检索不受 `use_search` 影响** —— 关掉联网也能「只用我自己的资料出题」。
   向量化确实要访问外部网络，但「关掉联网」关的是**公开检索**，不是一切网络请求。
...
| 位置 | 值 |
|---|---|
| 大厅冒险者卡 | ~~`Lv.3 见习冒险者` / `1280 / 2000 XP` / `连续 4 天` / 进度 64%~~ → **Phase F 已改为真实数据**（`GET /users/me`，见下） |
| 复习建议第 2 条 | `3 天后自动提醒复习` / `已加入复习计划` |
| 报告复习建议第 3 条 | `建议接着学「RAG 的落地实践」` |

> 其中**本局**的 XP、金币、正确率、用时是真实计算的，只有跨局的身份数据是演示值。

> **后记（Phase F）**：上表第一行已作废。用户系统落地后，大厅那张卡的等级 / 头衔 /
> 累计 XP / 下一级门槛 / 进度百分比 / 连续天数全部取自 `GET /users/me`
> （服务端 `level_service.py` 计算，前端只做格式化 `HALL_PROFILE_COPY`），
> `constants/mock.ts` 里的 `MOCK_ADVENTURER` / `MOCK_LEVEL_PERCENT` 已删除。
> 原「不做真实用户数据」的前提正是本节标题所依赖的，该前提已不成立 ——
> 保留本节其余两行是因为它们仍在演示范围内。
