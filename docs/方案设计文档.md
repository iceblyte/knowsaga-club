...
| `IMAGE_SELECT_ENABLED` / `IMAGE_SELECT_TIMEOUT_SECONDS` | `true` / `12` | 生图**之前**先判断哪几道题值得配图（2026-09-26 新增，design D18）—— 只给挑中的题花钱。⚠️ **判断失败会回退成「全部配图」**（它是省钱手段，不是功能前提，用户勾了就该拿到图）；关掉即回到「每道题都配」。它不参与 `image_generation_available` 的判定 |
...
| `IMAGE_PROMPT_EXTEND` / `IMAGE_WATERMARK` / `IMAGE_NEGATIVE_PROMPT` | `false` / `false` / 一组中文词 | 润色开关、浮水印、负向提示词。⚠️ **后两者不在 z-image-turbo 的官方参数表内**（官方只列 `size` / `prompt_extend` / `seed`）：实测带上仍是 200，但**上游是否真采用未经验证** —— 别把它们当有效护栏。而「画面不出现文字」的抓手**也不是任何一句正向禁令**（实测：把「不得出现任何文字」写进提示词反而会被画出来），是 `prompt.py` 的**提示词形态**（单段画面描述 + 静物风格词 + 摘除字母 token），且**只能靠人眼看图验收** |
| `IMAGE_MAX_WORKERS` / `IMAGE_TIMEOUT_SECONDS` / `IMAGE_BUDGET_SECONDS` | `2` / `20` / `45` | 并发数、**单张**超时、**整段**总预算（超预算即放弃剩余图片，不阻塞落库）。并发默认 **2**（2026-09-26 由 3 下调）：实测百炼账号约 **2 rps**，「并发 3 必挂 1 个 429」而「并发 2 连跑 8 张全过」，且客户端**无重试** ⇒ 撞 429 就等于那题没图。⚠️ 这是**账号配额**、不是上游硬限制；更根治的做法是 429 退避重试（未做） |
