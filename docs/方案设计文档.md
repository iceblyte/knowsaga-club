...
- **路由无条件注册**，不挂 `KNOWLEDGE_BASE_ENABLED`：挂开关会让关掉时前端拿到 404，
  与「路径写错了」长得一模一样。⚠️ 顺带一件**必须如实说明**的事：截至本次交付，那个开关
  **没有任何运行时分支读它** —— 省下 `import chromadb` 与 198 MB 的是 `store.py` 的
  **惰性导入**，不是这个键；取材链是否绑 `kb_search` 只看「这次请求有没有带 `kb_id`」
  （`llm/kb/tools.build_kb_tool` 只判 `request.kb is None`）；前端入口也未接它。
  也就是说它是**声明性**的，真正的闸门是 `DASHSCOPE_API_KEY`（缺它 → 文档落 `failed`）。
...
| 键 | 默认 | 作用 |
|---|---|---|
| `KNOWLEDGE_BASE_ENABLED` | `false` | **声明性**开关：当前无运行时分支读它（端点无条件注册、取材链只看 `kb_id`、前端入口未接）。真正的闸门是下一行 |
...
- **多份文档并发解析**：`KB_PARSE_WORKERS=2`「池满即判失败」这条路径只在单测里覆盖，
  没有在真环境同时上传多份大文档压过。
- **`.pdf` / `.docx` / `.txt` 三种真实文件**：实测只跑了 `.md`，另外三种格式只有单测覆盖。
- 测试里 embedding **一律注入假实现**（与「LLM 一律 mock」同一条红线），这是刻意的 ——
