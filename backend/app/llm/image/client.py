...
⚠️ **同样别再把「正向约束」当护栏**（2026-09-26 改口，见 design D16）。
旧版提示词把「画面中不得出现任何文字」当成约束写进正向通道，实测**适得其反** ——
模型把这句禁令本身、以及编号列表、冒号标签全渲染进了画面。
「画面不出现文字」现在的抓手是 `prompt.py` 的**形态**（单段画面描述 +
静物风格词 + 摘除字母 token），而且它**只能靠人眼看图**验收，单测断不了。
