...
## v2 相对 v1 改了什么（`add-web-search-grounding` 第 9 组，design D11）

**只加一条约束，其余一个字不动。** 报告链吃的是「已被资料约束过的题库」，
真正的取材风险在出题链就处理完了 —— 所以这里不需要再谈资料、更不该把
第三方原文搬进来（`REPORT_HUMAN_TEMPLATE` 的四个槽位一个都不增）。

新增的约束是：**三句话总结不得引入本次题库之外的新知识**。
它防的不是幻觉，而是**越界补课**：模型看到「向量检索」这个知识点，
顺手把训练数据里关于向量数据库的一堆东西写进总结，用户会以为自己学过。

为什么不给资料原文（D11）：报告是**复述**，塞第三方原文只增加不确定性，
还会让报告链的输入体积不可控 —— 而报告链没有任何截断护栏。
...
- `three_line_summary`：恰好 **3 句**知识总结，每句一句话、不超过 40 字，
  ...
  第 3 句讲它在真实场景里怎么用。**内容只能来自本次题库** —— 不得引入本次没考到的
  新知识、新名词或新结论，也不要补充题库之外的例子：用户会以为自己学过。
