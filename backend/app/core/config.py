...
class Settings(BaseSettings):
    ...
    # 默认 **2**（2026-09-26 由 3 下调，design D17）：实测上游对该账号约 2 rps，
    # 「并发 3 必挂 1 个 429」而「并发 2 连跑 4 轮 8 张零 429」。⚠️ 429 在
    # `llm/image/client.py` 里**没有任何重试**，挂掉就等于那道题没有配图 ——
    # 降级本身是对的，但默认值掉 1/3 的图不可接受。所以取**验证过的 2**，
    # 而不是当初拍脑袋的 3（3 从未被验证过）。
    # ⚠️ 别把它读成「2 是上游硬限制」：那是**账号级 RPS 配额**，
    # 换账号/提配额后可以调高，但调之前先真跑一轮确认零 429。
    image_max_workers: int = 2
    ...
    # 正向提示词那边则**刻意不提**「文字」二字：同一天的实测（design D16）显示，
    # 把「画面中不得出现任何文字」写进正向提示词，反而让模型把这句禁令本身渲染进画面。
    # 现在真正的抓手是 `prompt.py` 的**形态**（单段画面描述 + 静物风格词 + 摘除字母 token），
    # 不是任何一句禁令。
